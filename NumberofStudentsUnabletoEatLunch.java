...
class NumberofStudentsUnabletoEatLunch {
    public int countStudents(int[] students, int[] sandwiches) {
        int count = 0;  //takes the track of how many comparisons are made 
        int top = 0;
        int front = 0;
        int end = sandwiches.length-1;
        int size = students.length;
        
        while(count < size) {        //if no of comparisons are equal to current size of queue then size of the queue is answer
        ...
        // return the size of the queue or count of students unable to eat
        return count;
    }
}
