...
class CountPrefixandSuffixPairsI {
    public int countPrefixSuffixPairs(String[] words) {
        ...
        for(int i=0;i<words.length-1;i++) {
            String presuff = words[i];
            for(int j=i+1;j<words.length;j++) {
                String word = words[j];
                if(isPrefixAndSuffix(presuff, word)) {
                    count++;
                }
            }
        }
    ...
    private boolean isPrefixAndSuffix(String presuff, String word) {
        return word.startsWith(presuff) && word.endsWith(presuff);
    }
}
